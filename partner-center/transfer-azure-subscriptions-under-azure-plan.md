...
- Os produtos do Marketplace de terceiros serão transferidos desde que os produtos passem pela verificação de qualificação do Marketplace.
    ...
    - Os produtos são baseados em não assinatura
    - O parceiro futuro deve trabalhar com o editor para garantir que ele está na lista de permitir a implantação do produto
    - Se nem todas essas condições são atendidas para transferir os produtos do Marketplace devem ser canceladas, as assinaturas do Azure transferidas e, em seguida, a nova compra de produtos do Marketplace com o novo parceiro

**Pré-requisitos:**

- O cliente envolve o parceiro CSP atual em sua intenção de fazer a transição
- O parceiro CSP futuro funciona com o cliente para garantir que as necessidades do cliente possam ser atendidas
- O parceiro CSP futuro estabelece uma relação com o cliente e compra um plano do Azure antes do início da transição  
- O cliente deve assinar Contrato de Cliente da Microsoft com o parceiro CSP futuro
- O futuro parceiro CSP deve ter assinado o Contrato de Parceiro da Microsoft para usar essa ferramenta

## <a name="customer-tasks-to-be-completed"></a>Tarefas do cliente a serem concluídas

Para transferir uma assinatura do Azure em um plano do Azure, o cliente deve iniciar o processo contatando seu parceiro atual. Eles devem coletar o nome e o domínio da empresa do parceiro atual para que seu parceiro futuro possa preencher o formulário de solicitação de transferência em seu nome.

O cliente também deve identificar as assinaturas que deseja transferir do parceiro atual. Você não pode alterar parceiros para assinaturas do Office 365, Enterprise Mobility Suite ou Microsoft Dynamics CRM.

>[!Note]  
>É responsabilidade do parceiro futuro concluir o formulário de solicitação de transferência que inicia o processo de transferência. A Microsoft não pode intervir em nome do cliente ou do parceiro atual. O cliente deve planejar trabalhar em estreita proximidade com seu parceiro futuro e atual para fazer a transição funcionar sem problemas.
...
1.  No menu Partner Center, selecione **Clientes** e, em seguida, selecione o cliente que você deseja preencher um formulário de solicitação de transferência em nome dele.
...
5.  Revise a solicitação de transferência. Selecione as assinaturas do Azure solicitadas a transferir.

>[!Note]  
> Antes de continuar, observe: você não terá mais acesso às assinaturas selecionadas.
> Você não será faturado para uso posterior.
...
7.  Exibir confirmação de aceitação da transferência.
   ...
   Depois que a transferência for concluída, as assinaturas incluídas na solicitação serão exibidas no plano do Azure do parceiro futuro e não serão mais listadas com você.

>[!Note]  
>Para Provedores Indiretos: informe ao revendedor indireto que a solicitação de transferência foi aceita.

### <a name="managing-your-transferred-customer-subscriptions"></a>Gerenciando suas assinaturas de cliente transferidas

- O acesso a usuários, grupos ou entidades de serviço existentes que foram atribuídos usando o RBAC (controle de acesso baseado em função) do Azure não é afetado durante a transição. O [RBAC (controle](/azure/role-based-access-control/overview) de acesso baseado em função) do Azure ajuda o cliente a gerenciar quem tem acesso aos recursos do Azure, o que eles podem fazer com esses recursos e a quais áreas eles têm acesso. Como o novo parceiro, você não recebe acesso RBAC aos recursos do cliente após a transferência da assinatura. O parceiro anterior do cliente retém o acesso RBAC. Trabalhe com seu cliente para entender quem tem informações sobre suas assinaturas e como fazer as alterações que você quer.

- Consequentemente, é importante que seu cliente remova o acesso do RBAC do Azure para seu parceiro anterior e adicione acesso ao novo parceiro. Para obter mais informações sobre o cliente que oferece acesso novo, consulte [o que é o Azure RBAC (controle de acesso baseado em função)?](/azure/role-based-access-control/overview) Para obter mais informações sobre o cliente que remove o acesso RBAC do parceiro anterior, consulte [remover uma atribuição de função](/azure/role-based-access-control/role-assignments-portal#remove-a-role-assignment).
