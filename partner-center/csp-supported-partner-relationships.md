...
**Parceiros de cobrança direta** são parceiros com uma relação de cobrança direta com a Microsoft. Como parceiro de cobrança direto, você pode comprar ofertas de terceiros e ofertas de terceiros diretamente do Microsoft Marketplace e, em seguida, vendê-las aos seus clientes. Você precisa atender a determinados requisitos de programação para a função de parceiro de cobrança direto. Isso inclui a capacidade de integrar com a Microsoft usando APIs. Você também precisa ser capaz de fornecer serviços gerenciados, de cobrança e de suporte aos clientes de forma contínua. Para obter mais informações, consulte [registrar-se como parceiro de cobrança direta](enrolling-in-the-csp-program.md#enroll-as-a-direct-bill-partner).

## <a name="csp-program-requirements-signing-the-microsoft-partner-agreement-mpa"></a>Requisitos do programa CSP: assinando o Microsoft Partner Agreement (MPA)
...
O cliente final pode comprar serviços diferentes de parceiros diferentes, com base nas necessidades comerciais do cliente. Para obter mais informações, consulte [suporte a vários parceiros](multipartner.md).

**Cenário de exemplo 3:** E se um parceiro for contatado por um cliente que deseja contrata-los para gerenciar e dar suporte às suas assinaturas?

O cliente pode envolver vários parceiros para gerenciar suas assinaturas. Para obter mais informações, consulte [mult-Channel support](multichannel.md).

**Cenário de exemplo 4:** E se um parceiro no programa CSP (um provedor indireto, revendedor indireto ou parceiro de cobrança direto) quiser comprar ofertas da Microsoft ou de terceiros para seu próprio uso (como um cliente final)? O parceiro pode comprar essas ofertas de outro parceiro do programa CSP?

O parceiro pode comprar como um cliente de um revendedor indireto ou de um parceiro de cobrança direta. Para fazer isso, no entanto, eles devem usar um locatário diferente daquele usado para o CSP.

Nesse caso, os serviços comprados pelo parceiro precisam ser provisionados para um locatário designado como um tipo de cliente (um locatário somente para uso interno). Esses serviços **não podem** ser provisionados para o locatário do CSP do parceiro existente (o locatário usado pelo parceiro no programa do provedor de soluções na nuvem).</br>

**Cenário de exemplo 5:** E se um parceiro no programa CSP quiser vender para si mesmo como um cliente final?

Por contrato, os parceiros no programa CSP não têm permissão para vender ofertas da Microsoft ou de terceiros para si mesmos (como clientes finais) ou para suas organizações afiliadas (como clientes finais).

**Cenário de exemplo 6:** E se um revendedor indireto quiser comprar como um cliente final?

Alguns revendedores indiretos podem querer comprar licenças para seu próprio uso de um provedor indireto no programa CSP. Para esse cenário, a organização do revendedor indireto pode já ter estabelecido um relacionamento indireto do revendedor indireto do provedor. Isso impede que o parceiro estabeleça qualquer novo tipo de relação como um cliente final com o mesmo provedor indireto.

Nesse caso, o revendedor indireto pode usar uma das seguintes opções:

- **Opção 1:** Os parceiros podem configurar outro locatário do Azure AD que seja um ambiente separado para seus clientes.

- **Opção 2:** O revendedor indireto pode finalizar a relação com um provedor indireto e estabelecer uma nova relação de CSP com outro provedor indireto. Isso permite que o revendedor indireto compre assinaturas para o próprio uso da empresa de um parceiro de cobrança direto de CSP diferente.

   >[!NOTE]
   >Para finalizar a relação entre um provedor indireto e um revendedor indireto, entre em contato com [suporte da Microsoft](support-from-microsoft.md).

- **Opção 3:** O revendedor pode manter sua relação de clientes com o provedor indireto existente e estabelecer outra relação de revendedor indireto com outro provedor indireto.

**Cenário de exemplo 7:** E se um revendedor indireto quiser vender para um cliente que está em uma região diferente?

Um parceiro no programa CSP só pode vender para clientes da mesma região. Para obter mais informações, consulte [programas do provedor de soluções de nuvem mercados regionais e moedas](regional-authorization-overview.md).

**Cenário de exemplo 8:** E se um provedor indireto quiser estabelecer uma relação com um revendedor indireto em uma região diferente?
...
- [Registrar como um parceiro de cobrança direta](enrolling-in-the-csp-program.md#enroll-as-a-direct-bill-partner)